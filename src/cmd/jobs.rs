...
pub async fn post_job(form: web::Form<JobData>, data: Data<AppData>, identity: Option<Identity>) -> impl Responder{
    ...
    sole_query(&mut db, surrealql, Some(JobUsername{ job, username })).await.unwrap();
...
pub async fn jobs_data(data: Data<AppData>, path: web::Json<String>) -> impl Responder{
    ...
    let mut res1 = query_once::<JobPost>(&mut *data.db.lock().await, r#"SELECT * FROM jobs WHERE id=type::thing("jobs", $id) FETCH user.accounts;"#, ("id", path)).await.unwrap();
    let len = res1.len();
    //there should never be more than one but just in case
    if len != 1 {
        //bad request 
    }
    let job = res1.get_mut(0).unwrap();
    job.timestamp_converted().unwrap();
...
pub async fn tasks_in_area(app_data: Data<AppData>, js: web::Json<String>) -> impl Responder{
    ...
    let mut res2 = query_once::<JobPost>(&mut *app_data.db.lock().await, "SELECT * FROM jobs WHERE data.location = type::string($location) FETCH user.accounts;", ("location", address)).await.unwrap();
