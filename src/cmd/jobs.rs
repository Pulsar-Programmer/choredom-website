...
pub async fn jobs_data(data: Data<AppData>, path: web::Json<String>) -> impl Responder{
    ...
    let mut res2 = query::<JobPost>(&mut *data.db.lock().await, r#"SELECT * FROM jobs WHERE id=type::thing("jobs", $id) FETCH user.accounts;"#, ("id", path)).await.unwrap();
    let res1 = res2.get_mut(0).unwrap().as_mut().unwrap();
    let len = res1.len();
...
pub async fn tasks_in_area(app_data: Data<AppData>, js: web::Json<String>) -> impl Responder{
    ...
    let mut res2 = query::<JobPost>(&mut *app_data.db.lock().await, "SELECT * FROM jobs WHERE data.location = type::string($location) FETCH user.accounts;", ("location", address)).await.unwrap();
